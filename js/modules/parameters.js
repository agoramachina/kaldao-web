...
export class ParameterManager {
    constructor() {
        // Parameters - enhanced with larger kaleidoscope segments range
        this.parameters = {
            fly_speed: { value: 0.25, min: -3.0, max: 3.0, step: 0.1, name: "Fly Speed" },
            contrast: { value: 1.0, min: 0.1, max: 5.0, step: 0.1, name: "Contrast" },
            kaleidoscope_segments: { value: 10.0, min: 4.0, max: 200.0, step: 2.0, name: "Kaleidoscope Segments" },
        ...
        // Audio modifiers - these multiply the base parameters for audio reactivity
        this.audioModifiers = {
            fly_speed: 1.0,
            contrast: 1.0,
    ...
    // Get parameter value (with audio modifier applied)
    getValue(key) {
        const baseValue = this.parameters[key]?.value ?? 0;
        const modifier = this.audioModifiers[key] ?? 1.0;
        
        // Apply bounds checking for modified values
        ...
        if (param) {
            ...
            if (key === 'layer_count') {
                modifiedValue = Math.max(1, Math.min(10, modifiedValue));
            } else if (key === 'kaleidoscope_segments') {
                modifiedValue = Math.max(param.min, Math.min(param.max, modifiedValue));
                modifiedValue = Math.round(modifiedValue / 2) * 2; // Ensure even numbers
            } else {
                modifiedValue = Math.max(param.min, Math.min(param.max, modifiedValue));
            }
    ...
    // Get base parameter value (without audio modifier)
    getBaseValue(key) {
        return this.parameters[key]?.value ?? 0;
    }

    // Set parameter value (base value, not modified)
    setValue(key, value) {
        const param = this.parameters[key];
        if (param) {
            ...
            if (key === 'kaleidoscope_segments') {
                param.value = Math.round(param.value / 2) * 2; // Ensure even numbers
            }
        }
    }
    ...
    // Set audio modifier for a parameter
    setAudioModifier(key, modifier) {
        if (this.audioModifiers.hasOwnProperty(key)) {
            this.audioModifiers[key] = modifier;
        }
    }

    // Reset all audio modifiers
    resetAudioModifiers() {
        Object.keys(this.audioModifiers).forEach(key => {
            this.audioModifiers[key] = 1.0;
        });
    }
    ...
    // Update time accumulation with audio modifiers
